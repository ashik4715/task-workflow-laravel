    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">
                Manage permissions for user: <span class="text-indigo-600">{{ $user->name }}</span>
            </h3>
            <p class="mt-1 max-w-2xl text-sm text-gray-500 dark:text-gray-400">
                Assign direct permissions for this user. Role-based permissions are shown as pre-selected when applicable.
            </p>
        </div>
                                    ...
                                    <div class="flex items-start">
                                        <div class="flex items-center h-5">
                                            <input type="checkbox" name="permissions[]" value="{{ $perm->id }}"
                                                id="permission_{{ $perm->id }}" {{ in_array($perm->id, $displayPermissions) ? 'checked' : '' }}
