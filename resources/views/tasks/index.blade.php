    ...
    <div class="bg-white rounded-lg shadow mb-6">
        <div class="p-4 border-b border-gray-200">
            <form method="GET" action="{{ url('/tasks') }}" class="flex flex-wrap gap-4">
                ...
                <a href="{{ url('/tasks') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium">
                    Clear
                </a>
            </form>
        </div>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-200">
                                ...
                                <th class="text-left py-3 px-4 text-sm font-semibold text-gray-600">Owner</th>
                        ...
                        <tbody>
                            @foreach($tasks as $task)
                                <tr class="border-b border-gray-100 hover:bg-gray-50">
                                    ...
                                    <td class="py-3 px-4 text-sm text-gray-600">{{ $task->user->name }}</td>
                                    <td class="py-3 px-4">
                                        @switch($task->status)
                                            @case('PENDING')
                                    ...
                                    <td class="py-3 px-4 text-sm text-gray-500">{{ $task->created_at->format('M d, Y') }}</td>
                                    <td class="py-3 px-4">
                                        <a href="{{ url('/tasks/' . $task->id) }}" class="text-blue-600 hover:text-blue-700 text-sm">View</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
